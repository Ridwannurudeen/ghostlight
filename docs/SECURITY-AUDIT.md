...
| RI-10 | RI | HIGH | `src/server/state.ts`; `src/server/storage.ts`; `src/server/server.ts` | FIXED (current candidate) — before a final guess or post changes state, the server synchronously stores one bounded active-mutation journal containing validated after-images and the exact response. Hydration validates and reapplies that record before accepting users, checkpoints every derived charade, index, player-stat, decoder, and board target, then moves the operation into a 64-entry byte-bounded completion history. Exact retries recover the original response without double-counting. Partial-cut, malformed-journal, restart-replay, capacity, concurrent-decoder, and midnight-queue regressions cover the recovery path. |
...
By report: MV has 8 fixed; RI has 14 fixed and 3 accepted; RF has 11 fixed and 2 accepted.

## Accepted residual risk

The remaining storage risks are the pinned SDK's per-key read-error ambiguity, bounded write unavailability when an
unrelated historical dirty key fails permanently, and unsafe rollback across schema versions. Raw values,
collections, dirty bytes, host work, handler concurrency, replay state, and the durable mutation journal all have
hard ceilings; corrupt foundations become read-only; acknowledgements wait for checkpoints. A crash can leave a
temporary subset of one journaled operation's targets on Storage, but the validated journal reapplies the exact
after-images before the next server accepts users. The production cold-recovery gate below remains required because
the local harness cannot certify host Storage behavior.
...
- Production Storage latency, swallowed-error behavior, cold-wake journal replay after a partial write, and abrupt
