...
| RF-02 | RF | HIGH | `src/server/server.ts:919,992`; `src/server/state.ts:490,570` | FIXED (`ee5c75b`) — same stable-identity and guest exclusion controls as MV-04, including playbill, hardest ghosts, decoder rows, titles, stamps, and persistent content. |
